...
## 2026-09-22 · M0.5 — the hand in text, and the island

**Built** (receipt `receipts/M0.5_GATE_2026-09-22.md`): the hand connector (forward parser, mbox and IMAP backends, the paste folder); capture states with the sealed receipt and the typed reply; the three grade cases with `declared`/`passive` strata; the note as testimony under its own question set; UNRESOLVED as the fourth stock with O8c and lie arm 4; the unresolved re-join each tick, keyed only; `depends_on` on every derived object with cone invalidation; time as a reducer (past cells lose capacity); island mode with held effects and a verify that refuses the lie arm; the gym's forwards, pastes and future mail; 11 new tests (40 total).

**Traps, the day they bit (continued).**
13. **`ingested_ns` in the observation's content address.** The first tick re-admitted every forward and paste (six captures became thirty-six, and the "resolution" on tick 5 resolved five copies of one paste): the same external item got a new id each tick because the receipt time was in the preimage. `ingested_ns` and the derived `injection_shape` are now excluded from `Observation.with_id()`; ids are stable across ticks and runs. This is a version event on observation ids (L22): no persisted real tape existed yet; the gym regenerates.
14. **Past slots kept their capacity.** Within a day the field barely moved between six-hour ticks (‖Δv‖ 0.003) because the slot grid is day-anchored and nothing removed the cells before `now`. Time consuming slack means those cells are gone: the bridge now zeroes capacity before `now`, and a six-hour tick moves the prices by 15–30 with no message (62 when a mail lands and a capture resolves). This is L16's "the field must still move" made mechanical, and it was not mechanical until it bit.
15. **The audit measured time-to-last-row.** A capture RESOLVED thirty hours after sealing failed a one-hour budget. The budget bounds time to the FIRST terminal state; RESOLVED is a transition out of UNRESOLVED, not a delay. `hand_audit` now records the first terminal row per capture.
16. **The scorer did not know the hand plants**, so M0's e2e precision fell to 0.7 on the regenerated gym: three UNRESOLVED_CAPTURE findings were true by construction and counted as spurious. `gate_m0.HAND_TRUE` names them by marker; recall on the mail plants is scored apart.
17. A second DUPLICATE candidate (Priya, cos 0.63) appeared at the 0.60 floor; raised to 0.75 as a placeholder above the gym's noise. The trough is measured on real data (B10), never inherited.

**Measured.** The island run: 24 synthetic hours in four ticks, 941 rows, 10.4 s on the CPU reference; `on` 1, `NETWORK_ABSENT` 11, `FRONTIER_ABSENT` 4; 6 effects held with inverses; ‖Δv‖ per tick 17.7 / 21.5 / 31.8 / 14.5; replay byte-identical; the lie arm refused. The hand on the gym: every one of six plants reached its expected state and grade case; the quote reference resolved by exact id on the tick that admitted the future mail; the stock prices UNPLACED 0 · UNADJUDICATED 5.26 · WAITING 4.03 · UNRESOLVED 3.30. Instrument: 9/9 oracles on three compilers; lies 1, 3, 4 caught.

**Decided.** (i) Sealing is admission, never gated (D6). (ii) A capture with no id and no named actor cannot be keyed and stays UNRESOLVED; the embedding proposes and never resolves. (iii) Replies are composed once at scan; a later resolution lives on the tape (B20). (iv) The IMAP leg is written and refused correctly on the island; the SMTP leg is unbuilt and the receipt says "file; smtp leg unbuilt" (B18). (v) In-process ticks after a scan; a rehydrating `tick` verb is B17.

## 2026-09-22 · M0, decisions (carried)
