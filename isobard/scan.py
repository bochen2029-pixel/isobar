...
Silence is measured against who spoke last on the thread: `last_owner` and `last_counter` map a
thread root to the last message time by the owner and by anyone else."""
...
def recovery_scan(now_ns: int, commitments: dict[str, Commitment], money: dict[str, MoneyObject], actors: dict[str, Actor],
                  owner_id: str, last_owner: dict[str, int], last_counter: dict[str, int], notes: dict[str, dict],
                  duplicates: list[tuple[str, str, float]], quiet_days: float = 5.0) -> list[Finding]:
    out: list[Finding] = []
    ...
    for c in commitments.values():
        if c.state in ("discharged", "cancelled", "superseded") or c.kind == "appointment":
            continue
        counter = c.creditor_actor if c.debtor_actor == owner_id else c.debtor_actor
        cname, tier = actor_name(counter)
        root = c.thread_id or ""
        origin = c.release_ns or 0
        lo, lc = last_owner.get(root), last_counter.get(root)
        counter_after = lc is not None and lc > origin
        owner_after = lo is not None and lo > origin
        silent = _days(max(origin, lc or 0), now_ns) if counter_after else _days(origin, now_ns)
        note = notes.get(c.id, {})
        outbound = bool(note.get("outbound"))
        mo = money.get(c.money_object) if c.money_object else None
        base_why: dict[str, Any] = {"rule": note.get("rule"), "direction": note.get("direction"), "outbound": outbound,
                                    "p_promoted": round(c.p_promoted, 3), "p_hard": round(c.due.p_hard, 3), "thread": root,
                                    "state": c.state, "last_owner_after": owner_after, "last_counter_after": counter_after}

        if mo is not None and mo.id not in seen_money:
            seen_money.add(mo.id)
            if mo.kind in ("quote", "proposal") and mo.status in ("sent", "acknowledged", "unknown") and not counter_after and (silent or 0) >= quiet_days:
                out.append(Finding(finding_type="SILENT_PROPOSAL", commitment_id=c.id, counterparty=cname, counterparty_tier=tier,
                                   ...
                                   sent_ns=mo.issued_ns or origin, last_meaningful_reply_ns=lc, days_silent=silent,
                                   confidence=round(min(0.99, c.p_promoted + 0.1), 3), evidence=list(c.evidence), recommendation="FOLLOW_UP",
                                   why={**base_why, "money": mo.model_dump(), "silent_days": silent}))
                continue
            if mo.kind == "invoice":
                if mo.status == "paid":
                    continue  # the verdict said paid: never a finding, never a chase (K_PAID)
                if mo.status in ("sent", "unknown", "overdue"):
                    due = mo.due_ns or (mo.issued_ns + 30 * DAY_NS if mo.issued_ns else None)
                    if due and due < now_ns:
                        connected = mo.status_source == "verdict_connector"
                        out.append(Finding(finding_type="APPARENT_OVERDUE_INVOICE", commitment_id=c.id, counterparty=cname, counterparty_tier=tier,
                                           amount_minor=mo.amount_minor, currency=mo.currency, payment_status_connected=connected,
                                           sent_ns=mo.issued_ns, last_meaningful_reply_ns=lc, days_silent=_days(due, now_ns),
                                           confidence=round(min(0.99, c.p_promoted), 3), evidence=list(c.evidence),
                                           recommendation="FOLLOW_UP" if connected else "REVIEW",
                                           why={**base_why, "money": mo.model_dump(),
                                                "label": "confirmed by the verdict source" if connected else "payment status not connected — not upgraded to money owed"}))
                        continue

        if c.debtor_actor == owner_id and outbound and c.due.latest_ns and c.due.latest_ns < now_ns:
            out.append(Finding(finding_type="PROMISE_PAST_DUE", commitment_id=c.id, counterparty=cname, counterparty_tier=tier,
                               sent_ns=origin, last_meaningful_reply_ns=lc, days_silent=_days(c.due.latest_ns, now_ns),
            ...
            continue
        if c.debtor_actor == owner_id and not outbound and not owner_after and (silent or 0) >= quiet_days:
            ...
            out.append(Finding(finding_type="CUSTOMER_WAITING", commitment_id=c.id, counterparty=cname, counterparty_tier=tier,
                               sent_ns=origin, last_meaningful_reply_ns=None, days_silent=silent,
                               confidence=round(min(0.99, c.p_promoted * (0.5 + 0.5 * float(tw[1]))), 3), evidence=list(c.evidence),
                               recommendation="REPLY", why={**base_why, "they_wait": list(tw)}))
            continue
        if c.debtor_actor != owner_id and outbound and not counter_after and (silent or 0) >= quiet_days:
            ...
            out.append(Finding(finding_type="WE_ARE_WAITING", commitment_id=c.id, counterparty=cname, counterparty_tier=tier,
                               sent_ns=origin, last_meaningful_reply_ns=lc, days_silent=silent,
                               confidence=round(min(0.99, c.p_promoted * (0.5 + 0.5 * float(ww[1]))), 3), evidence=list(c.evidence),
                               recommendation="FOLLOW_UP", why={**base_why, "we_wait": list(ww)}))
            continue
    ...
    for a, b, cs in duplicates:
        ...
        out.append(Finding(finding_type="DUPLICATE", commitment_id=a, counterparty=cname, counterparty_tier=tier, confidence=round(cs, 3),
                           ...
                           why={"pair": [a, b], "cosine": round(cs, 3), "a": ca.deliverable_text[:80], "b": cb.deliverable_text[:80],
                                "note": "candidate; a sampled audit grades precision; never merged on similarity alone"}))
...
def xray_text(findings: list[Finding], partition: dict[str, int], coverage: str, now_label: str) -> str:
    ...
    for f in findings:
        ...
        age = f"silent {f.days_silent}d" if f.days_silent is not None else f"cos {f.confidence:.2f}"
        lines.append(f"  {f.finding_type:<26} T{f.counterparty_tier} {f.counterparty:<16}{amt}{lab}  {age}  p={f.confidence:.2f}  → {f.recommendation}")
    return "\n".join(lines)
